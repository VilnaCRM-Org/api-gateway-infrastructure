...
def main() -> None:
    ...
    parser.add_argument("--backend-url", default=os.environ.get("PULUMI_BACKEND_URL"))
    ...
    try:
        ...
        login_cmd = ["pulumi", "login"]
        if backend_url:
            login_cmd.append(backend_url)
        login = run_command(login_cmd, cwd=project_dir, env=base_env, capture_output=True)
